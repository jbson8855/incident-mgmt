...
export type IncidentCalculationOutcome =
  | { ok: true; result: IncidentCalculationResult }
...
export function calculateIncident(body: IncidentCalculationRequest | null): IncidentCalculationOutcome {
  if (!body || typeof body.companyId !== 'number') {
    return { ok: false, status: 400, error: 'companyId가 필요합니다.' };
  }
  if (typeof body.title !== 'string' || !body.title.trim()) {
    return { ok: false, status: 400, error: '장애 제목을 입력해주세요.' };
  }
  ...
  const company = db.prepare('SELECT id, name, created_at FROM companies WHERE id = ?').get(body.companyId) as
    ...
    | undefined;
  if (!company) {
    return { ok: false, status: 404, error: '계열사를 찾을 수 없습니다.' };
  }
  ...
  if (!businessCategory || !complexityCategory || !customerCategory) {
    return { ok: false, status: 400, error: '기준표가 완전하지 않습니다.' };
  }
  ...
  if (invalidSystemIds.length > 0) {
    return { ok: false, status: 400, error: '선택한 시스템 중 존재하지 않는 항목이 있습니다.' };
  }
  if (selectedSystemItemIds.length === 0) {
    return { ok: false, status: 400, error: '영향받은 시스템을 입력해주세요.' };
  }
  ...
  if (failureScopeResolved.error) {
    return { ok: false, status: 400, error: failureScopeResolved.error };
  }
  ...
  if (validationErrors.length > 0) {
    return { ok: false, status: 400, error: validationErrors.join(' ') };
  }
  ...
  if (gradeThresholds.length === 0) {
    return { ok: false, status: 400, error: '장애등급 판정 기준이 없습니다.' };
  }
